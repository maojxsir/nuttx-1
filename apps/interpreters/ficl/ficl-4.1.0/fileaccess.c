...
static void ficlFileOpen(ficlVm *vm, char *writeMode) /* ( c-addr u fam -- fileid ior ) */
{
    ...
    *mode = 0;
    ...
    if (f == NULL) 
	 ...
	 printf("open-file:file(%08x) open flags:%x\n",f,f->fs_oflags);
    pushIor(vm, f != NULL);
...
static void ficlPrimitiveIoctl(ficlVm *vm)
{
    void     *args = (void*)ficlStackPopPointer(vm->dataStack);
    int       cmd = ficlStackPopInteger(vm->dataStack);
    ficlFile *ff = (ficlFile*)ficlStackPopPointer(vm->dataStack);

    ficlStackPushInteger(vm->dataStack,ioctl(ff->f->fs_fd,cmd,args));
}
...
static void ficlPrimitiveWriteFile(ficlVm *vm) /* ( c-addr u1 fileid -- ior ) */
{
    ...
    clearerr(ff->f);
	printf("$$write file:f:%08x flags:%x\n",ff->f,ff->f->fs_oflags);
...
void ficlSystemCompileFile(ficlSystem *system)
{
    ...
    ficlDictionarySetPrimitive(dictionary, "ioctl", ficlPrimitiveIoctl, FICL_WORD_DEFAULT);
